...
EmotePtr CustomBadgesController::getOrCreateEmote(const QString &imageFilePath, const QString &badgeName)
{
    ...
    QString displayName = badgeName.isEmpty() ? QString("Custom Badge") : badgeName;
    ...
    const TwitchBadge &badge, const QString &channelName)
{
    ...
    {
        if (cb.isAddon()) continue; // We only want replacement badges here
        ...
        if (!cb.channelName().isEmpty() && cb.channelName().compare(channelName, Qt::CaseInsensitive) != 0)
            continue;
    ...
    const std::vector<TwitchBadge> &badges, const QString &channelName)
{
    ...
    for (const auto &cb : *snapshot)
    {
        if (!cb.isAddon()) continue; // We only want addon badges here
        ...
        {
            matched = true; // matches any user (maybe apply to everyone in channel)
        }
        else
        {
            for (const auto &userBadge : badges)
            {
                bool typeMatches = cb.badgeType().isEmpty() || cb.badgeType() == userBadge.key_;
                bool versionMatches = cb.badgeVersion().isEmpty() || cb.badgeVersion() == userBadge.value_;
        ...
        if (matched)
        {
            result.push_back(this->getOrCreateEmote(cb.imageFilePath(), cb.name()));
        }
    }
