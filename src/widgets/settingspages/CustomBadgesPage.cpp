...
CustomBadgesPage::CustomBadgesPage()
{
    ...
    auto *checkbox = layout.emplace<QCheckBox>("Enable Custom Badges").getElement();
    ...
    layout.emplace<QLabel>(
        "Manage your custom badges. 'Replacement' badges override existing Twitch badges "
        "(e.g. subscriber, moderator).\n'Addon' badges appear alongside existing badges "
        "when the channel and matching criteria are met.\nBadge images should be at least 18x18 pixels for best results.");

    auto *model = new CustomBadgeModel(this);
    model->initialized(&getSettings()->customBadges);

    EditableModelView *view = layout.emplace<EditableModelView>(model).getElement();
    this->view_ = view;

    view->setTitles({"Mode", "Name", "Type/Version", "Restriction", "Image", "Channel"});
    ...
    QObject::connect(view->getTableView(), &QTableView::doubleClicked, [this](const QModelIndex &index) {
    ...
    QTimer::singleShot(100, [view] {
        ...
        view->getTableView()->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
    });
}
...
void CustomBadgesPage::showEditDialog(const CustomBadge *badge, int row)
{
    ...
    // Connect "Browse"
    QObject::connect(browseBtn, &QPushButton::clicked, [&dialog, imageEdit, badgesDir] {
        (void)QDir().mkpath(badgesDir);
        QString filePath = QFileDialog::getOpenFileName(&dialog, "Select Custom Badge Image", badgesDir, "Images (*.png *.gif *.jpeg *.jpg)");
        if (!filePath.isEmpty())
        {
            ...
            else
            {
                ...
                {
                    QString newFileName = baseName + "_" + QString::number(counter) + "." + suffix;
    ...
    if (dialog.exec() == QDialog::Accepted)
    {
        ...
        CustomBadge newBadge(
            type,
            version,
            channelEdit->text().trimmed(),
            restrictionEdit->text().trimmed(),
